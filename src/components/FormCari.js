...
const FormCari = ()=>{
    ...
    const fetchCars = async ()=>{
        ...
        .then(cars => {
            setCars(
            cars.filter(car=>{
                return (new Date(Date.parse(car.availableAt.split('T')[0])) < new Date(state.tanggal_tersedia) && `${new Date(Date.parse(car.availableAt)).getHours()}:${new Date(Date.parse(car.availableAt)).getMinutes()}` == state.waktu_jemput) && (car.capacity>=state.jumlah_penumpang  || (state.tipe_driver == 1 ? car.available === true : car.available === false))
            }))
            setIsLoad(false)
            setIsFocus(false)
        })
        ...
        <div className="card justify-content-between">
            <div className="row gx-2">
              ...
              <div className="col-lg">
                  ...
                  <input type="date" className="form-control" placeholder="Tanggal" id="tanggal" name="tanggal_tersedia" value={state.tanggal_tersedia} onChange={handleInput}/>              
                </div>     
             <div className="col-lg">
                  <label  className="form-label">Waktu Jemput/Ambil</label>
               <input type="time" className="form-control form-item" id="waktu-jemput" name="waktu_jemput" value={state.waktu_jemput} onChange={handleInput}/>              
                </div>    
              <div className="col-lg">
                ...
                id="jumlah-penumpang" name="jumlah_penumpang" value={state.jumlah_penumpang} onChange={handleInput}/>
            </div>
              <div className="col-sm">
                <div className="search-button">
                  <button type="submit" id="load-btn" className="btn btn-cari" onClick={fetchCars}>
                    Cari Mobil
                  </button>
